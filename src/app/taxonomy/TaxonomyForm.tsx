...
export function TaxonomyForm({ type, item, onSuccess }: TaxonomyFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    if (item) {
      await updateTaxonomy(type, item.id, formData)
    } else {
      await createTaxonomy(type, formData)
    }
  ...
  return (
    <div className="bg-gray-50 p-6 rounded-lg mb-6">
      <h3 className="mb-4 text-lg font-semibold">
        {isEditing ? `Edit ${typeLabels[type]}` : `Add New ${typeLabels[type]}`}
      </h3>
      <form
        ref={formRef}
        action={handleSubmit}
