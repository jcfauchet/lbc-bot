...
import { deleteTaxonomy, toggleTaxonomyStatus } from './actions'
...
export function TaxonomyList({ type, items, onEdit }: TaxonomyListProps) {
  ...
  const handleToggle = async (id: string) => {
    await toggleTaxonomyStatus(type, id)
    router.refresh()
  }

  return (
    <div className="mb-8">
      <h3 className="text-xl font-semibold mb-4">{typeLabels[type]}</h3>
      <div className="grid gap-3">
        ...
        {items.length === 0 && (
          <p className="text-center text-gray-600 italic py-4">
            No {typeLabels[type].toLowerCase()} yet. Add one above!
          </p>
        )}
      </div>
    </div>
