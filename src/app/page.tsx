          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Valeur de revente estimée (gains potentiel)
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  Actions
                </th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {stats.latestAlerts.map((alert) => (
                <tr key={alert.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {alert.listing.aiAnalysis ? (
                      <div>
                        <div className="font-medium text-gray-900">
                          {(alert.listing.aiAnalysis.estMinCents / 100).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })} - {(alert.listing.aiAnalysis.estMaxCents / 100).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                        </div>
                        <div className={`text-xs ${alert.listing.aiAnalysis.marginCents >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {alert.listing.aiAnalysis.marginCents >= 0 ? '+' : ''}{(alert.listing.aiAnalysis.marginCents / 100).toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                        </div>
                      </div>
                    ) : (
